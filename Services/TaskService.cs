...
namespace Services
{
    public interface ITasksService
    {
        ...
        Task<IEnumerable<Models.Task>> GetGoalTasks(int GoalId);
    ...
    public class TaskSerivice : ITasksService
    {
        ...
        public async Task<IEnumerable<Models.Task>> GetGoalTasks(int goalId)
        {
            IEnumerable<BsonDocument> bsonTasks = _tasks.GetAllTasks();
            List<Models.Task> goalTasks = new List<Models.Task>();

            foreach (BsonDocument bson in bsonTasks)
            {
                bson.Remove("_id");
                Models.Task taskToCheck = BsonSerializer.Deserialize<Models.Task>(bson);
                if (taskToCheck.linkedGoal.GoalId == goalId) goalTasks.Add(taskToCheck);
            }
            return goalTasks;
        }
    }
